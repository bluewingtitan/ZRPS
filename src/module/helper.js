export class EntitySheetHelper {
  static getAttributeData(data) {
    ...
    for (let key of groupKeys) {
      ...
      if (!data.system.groups[key]["attributes"])
        data.system.groups[key]["attributes"] = {};
    ...
    const keys = Object.keys(data.system.attributes).filter(
      (a) => !groupKeys.includes(a),
    );
  ...
  static onSubmit(event) {
    ...
    if (event.currentTarget) {
      ...
      if (
        ...
        !event.currentTarget.hasAttribute("name")
      ) {
        return false;
      }
        ...
        for (let i = 0; i < groups.length; i++) {
          if (groups[i].value === val) {
            ui.notifications.error(
              game.i18n.localize("SIMPLE.NotifyAttrDuplicate") + ` (${val})`,
            );
            el.value = oldVal;
            attrError = true;
            break;
  ...
  static onAttributeRoll(event) {
    ...
    const chatLabel =
      label ?? button.parentElement.querySelector(".attribute-key").value;
    const shorthand = game.settings.get("zrps", "macroShorthand");

    // Use the actor for rollData so that formulas are always in reference to the parent actor.
    const rollData = this.actor.getRollData();
    let formula = button
      .closest(".attribute")
      .querySelector(".attribute-value")?.value;
  ...
  static validateGroup(groupName, document) {
    ...
    if (["attr", "attributes"].includes(groupName)) {
      ui.notifications.error(
        game.i18n.format("SIMPLE.NotifyGroupReserved", { key: groupName }),
      );
      return false;
    }
  ...
  static async createAttribute(event, app) {
    ...
    let objKeys = Object.keys(attrs).filter(
      (k) => !Object.keys(groups).includes(k),
    );
  ...
  static updateAttributes(formData, document) {
    ...
    const formAttrs =
      foundry.utils.expandObject(formData)?.system?.attributes || {};
      ...
      if (!v["key"]) {
        ...
        attrs.forEach((attrKey) => {
          ...
          const k = this.cleanKey(
            v[attrKey]["key"] ? v[attrKey]["key"].trim() : attrKey.trim(),
          );
          delete attr["key"];
  ...
  static updateGroups(formData, document) {
    ...
    const groups = Object.entries(formGroups).reduce((obj, [k, v]) => {
      const validGroup =
        documentGroups.includes(k) || this.validateGroup(k, document);
  ...
  static async createDialog(data = {}, options = {}) {
    ...
    const templates = collection.filter((a) => a.getFlag("zrps", "isTemplate"));
    const defaultType =
      this.TYPES.filter((t) => t !== CONST.BASE_DOCUMENT_TYPE)[0] ??
    ...
    return Dialog.prompt({
      ...
      callback: (html) => {
        ...
        if (template) {
          createData = foundry.utils.mergeObject(
            template.toObject(),
            createData,
          );
          createData.type = template.type;
          delete createData.flags.zrps.isTemplate;
        }
  ...
  static clampResourceValues(attrs) {
    ...
    for (const [attr, value] of Object.entries(flat)) {
      ...
      foundry.utils.setProperty(
        attrs,
        attr,
        Math.clamp(value, current.min || 0, current.max || 0),
      );
    }
  }
  ...
  static cleanKey(key) {
    ...
    if (clean !== key)
      ui.notifications.error("SIMPLE.NotifyAttrInvalid", { localize: true });
    return clean;
  }
}
